"""
speech/tts_engine.py — JARVIS OMEGA V11 ★ FIXED ★
FIXES:
 1. NO media player window opens — plays audio silently in background
 2. Uses subprocess with hidden window for PowerShell playback
 3. pyttsx3 as fully offline fallback (no window)
 4. Better text cleaning for TTS
 5. Queue-based non-blocking design
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# AUDIO PLAYBACK ★ FIXED — NO media player window ★
# Uses hidden subprocess for silent background playback
# ══════════════════════════════════════════════════════════════════════════════
def _play_audio(path: str):
    """Play audio file WITHOUT opening any media player window."""
    path = os.path.abspath(path)
    sys = platform.system()

    if sys == "Windows":
        # WAV → winsound (built-in, synchronous, NO window)
        ...
        # MP3 → Hidden PowerShell MediaPlayer (NO window popup)
        try:
            ps_cmd = (
                f"$player = New-Object System.Windows.Media.MediaPlayer; "
                f"$player.Open([uri]'file:///{path.replace('\\', '/')}'); "
                f"$player.Play(); "
            ...
            result = subprocess.run(
                ...
                timeout=120, capture_output=True, startupinfo=si
            )
        ...
        # Fallback: ffplay in background (no window if available)
        try:
            si = subprocess.STARTUPINFO()
            si.dwFlags |= subprocess.STARTF_USESHOWWINDOW
            si.wShowWindow = 0
...
# ══════════════════════════════════════════════════════════════════════════════
# PYTTSX3 (offline fallback — works without internet, NO window)
# ══════════════════════════════════════════════════════════════════════════════
_pyttsx_engine = None
...
class TTSEngine:
    ...
    NO media player window ever opens!
    """
    ...
    def _speak_one(self, text: str):
        """Try Edge TTS first, fall back to pyttsx3. NO window ever opens."""
        if EDGE:
            try:
